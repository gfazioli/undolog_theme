<?php get_header(); ?>
	...
	<?php
		$max = 4;
		$index = 0;
		$args = array( 'numberposts' => $max, 'category_name' => 'very-short-snippet', 'post__not_in' => $noduplicate);
		$lastposts = get_posts( $args );
		foreach($lastposts as $post) : setup_postdata($post); ?>
		<?php $noduplicate[] = $post->ID; ?>
		<h3><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
	...
	<div class="fourColumns">
		...
		<?php
			$max = 5;
			$index = 0;
			$args = array( 'numberposts' => $max, 'category_name' => 'objective-c-sviluppo', 'post__not_in' => $noduplicate);
			$lastposts = get_posts( $args );
			foreach($lastposts as $post) : setup_postdata($post); ?>
			<?php $noduplicate[] = $post->ID; ?>
			<h3><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
